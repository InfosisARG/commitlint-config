...
<!-- Title: Contributing GitFlow CommitlintConfig -->
...
To contribute to commitlint-config project on [GitHub](https://github.com/InfosisARG/commitlint-config), We use Gitflow (See more [here](https://datasift.github.io/gitflow/IntroducingGitFlow.html)) In a nutshell, it means that you should branch from the main repository and contribute back by making [pull request](https://github.com/InfosisARG/commitlint-config/pulls).

![workflow gitflow](https://datasift.github.io/gitflow/GitFlowHotfixBranch.png)

## Getting started

To follow the instructions in this guide and start contributing to commitlint-config project on Gitlab:
...
```bash
git remote add upstream  https://github.com/InfosisARG/commitlint-config.git
```

Now your **upstream** points to **InfosisARG/commitlint-config**.
...
git fetch upstream # assuming upstream is InfosisARG/commitlint-config
...
All merge requests are automatically tested using [Github Actions](https://github.com/InfosisARG/commitlint-config/actions). In case some tests fail, fix the issues or describe why the fix cannot be done.
